...
makedepends=('git')
depends=('bash' 'dosfstools' 'efibootmgr' 'which' 'iasl')
...
_prepare_tianocore_sources() {
	...
	cp -r "${srcdir}/${_pkgname}/" "${_UDK_DIR}/RefindPkg/"
	cd "${_UDK_DIR}/"
...
_prepare_refind_sources() {
	...
	if [[ "${_USE_GNU_EFI}" == '1' ]]; then
		...
		# Disable btrfs build
		sed 's|RefindPkg/filesystems/btrfs.inf|# RefindPkg/filesystems/btrfs.inf|g' -i "${srcdir}/${_pkgname}_build/RefindPkg/RefindPkg.dsc" || true
